...
export async function GET(request) {
  ...
  try {
    ...
    try {
      ...
      if (!pErr) projects = pData;
      if (!mErr) milestones = mData;
    } catch {
      // Fallback silently if offline or paused Supabase
    }
    ...
    let finalProjects = (projectsWithMilestones && projectsWithMilestones.length > 0) ? projectsWithMilestones : defaultProjects;
    if (status && status !== 'all' && (!projectsWithMilestones || projectsWithMilestones.length === 0)) {
      finalProjects = defaultProjects.filter((p) => p.status === status);
    }
