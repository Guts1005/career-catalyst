...
export function calculateCareerReadiness({
  ...
  // Check how many required skills are covered by at least one completed/in-progress project
  ...
  const resumeScore = resumeData?.atsScore || (resumeData?.full_name ? 75 : 50);
  ...
  const appScore = Math.min(Math.round(activeApplications * 5 + interviewCount * 15 + (assessments.length > 0 ? 20 : 0)), 100);
  ...
  return {
    overallScore: Math.min(Math.max(overall, 15), 98),
...
export function generateNextBestAction({ targetRole, skills = [], projects = [], jobs = [], readiness }) {
  ...
  if (biggestGap && biggestGap.delta >= 25) {
    return {
...
// ─── Default Benchmark Demo State (Consistent Across Entire App) ─────
export const BENCHMARK_DEMO_DATA = {
  profile: {
    name: 'Sharvin Patel',
    title: 'Machine Learning Engineer',
  ...
  skills: [
    { id: 1, name: 'PyTorch & CUDA', category: 'Machine Learning', current_level: 90, target_level: 95, evidence_level: 'VERIFIED' },
    { id: 2, name: 'Distributed Systems', category: 'Systems', current_level: 82, target_level: 90, evidence_level: 'PROJECT' },
    { id: 3, name: 'MLOps & Deployment', category: 'Infrastructure', current_level: 85, target_level: 90, evidence_level: 'VERIFIED' },
    { id: 4, name: 'Docker & Kubernetes', category: 'Infrastructure', current_level: 80, target_level: 85, evidence_level: 'PROJECT' },
    { id: 5, name: 'Vector Search (FAISS)', category: 'Data Systems', current_level: 92, target_level: 90, evidence_level: 'VERIFIED' },
    { id: 6, name: 'System Design', category: 'Systems', current_level: 78, target_level: 85, evidence_level: 'ASSESSED' },
  ],
  projects: [
    {
      id: 1,
      name: 'Triton Low-Latency Inference Gateway',
      description: 'Custom Triton C++ kernel serving Llama-3 with dynamic batching and P99 latency < 14ms under 5,000 req/sec load.',
  ...
  jobs: [
    { id: 1, company: 'Anthropic', role: 'ML Systems Engineer', location: 'San Francisco, CA', salary: '$210k - $270k', status: 'interview', match_score: 94 },
    { id: 2, company: 'NVIDIA', role: 'Inference Performance Engineer', location: 'Santa Clara, CA', salary: '$195k - $250k', status: 'oa', match_score: 91 },
    { id: 3, company: 'Cohere', role: 'Distributed Training Engineer', location: 'Remote', salary: '$185k - $240k', status: 'applied', match_score: 88 },
  ],
};
